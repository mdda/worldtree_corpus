# From https://github.com/umanlp/tg2019task/blob/master/baseline_tfidf.py
...
def read_explanations(path):
  ...
  return df.apply(
      lambda r:
      (r[uid], ' '.join(str(s) for s in list(r[header]) if not pd.isna(s))),
      1).tolist()
...
def main():
  ...
  for i_question, distances in enumerate(X_dist):
    for i_explanation in np.argsort(distances)[:args.nearest]:
      print('{}\t{}'.format(df_q.loc[i_question]['questionID'],
                            df_e.loc[i_explanation]['uid']))
