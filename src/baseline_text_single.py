...
class TextPairRetriever(Retriever):
    ...
    def run_query(self, text: str, statements: List[str]) -> Tensor:
        chunks = []
        for i in range(0, len(statements), self.bs):
            texts_b = statements[i : i + self.bs]
            texts_a = [text] * len(texts_b)
            inputs = self.ds.make_tokens(texts_a, texts_b)
            chunks.append(self.run_batch(inputs))
        return torch.cat(chunks, dim=0)

    def run_all(self, queries: List[str], statements: List[str]) -> np.ndarray:
        ...
        with torch.no_grad():
            for i in tqdm(range(len(queries))):
                scores[i] = self.run_query(queries[i], statements).cpu().numpy()

        return scores

    def rank(self, queries: List[str], statements: List[str]) -> np.ndarray:
        scores = self.run_all(queries, statements)
